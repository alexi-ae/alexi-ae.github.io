...
export function getCurrentTime(): Date {
  ...
  const now = new Date();
  // Convert the UTC time to Italy's time
  const offsetItaly = -5; // Italy is in Central European Summer Time (UTC+2), but you might need to adjust this based on Daylight Saving Time
  now.setHours(now.getUTCHours() + offsetItaly);
  return now;
}
...
export function formatTime(date: Date): string {
  ...
  let formattedTime = new Intl.DateTimeFormat("es-ES", options).format(date);
...
export function formatDateHere(date: Date): string {
